...
class Service:
    ...
    def start(self):
        print('[Collector Error Init] Sleeping...')
        time.sleep(configs.sleep_interval)
        print('[Collector Error Init] Init Successfully!')
        while True:
            try:
                r = redis.Redis(host=self.redis_host, port=self.redis_port)
                # 先拉人脸的结果
                mode = 'face_recognition'
                response_str = r.lpop(self.ERROR_KEY)

                if not response_str or response_str is None:
                    # 再拉说话人的结果
                    mode = 'speaker_recognition'
                    response_str = r.lpop(self.SPEAKER_ERROR_KEY)
