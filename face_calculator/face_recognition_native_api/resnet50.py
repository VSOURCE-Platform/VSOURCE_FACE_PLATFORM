...
def identity_block2d(input_tensor, kernel_size, filters, stage, block, is_training, reuse, kernel_initializer=None):
	...
	x = tf.layers.conv2d(x, filters2, kernel_size, padding='SAME', use_bias=False, name=conv_name_2, reuse=reuse, kernel_initializer=kernel_initializer)
...
def resnet50(input_tensor, is_training=True, pooling_and_fc=True, reuse=False, kernel_initializer=tf.contrib.layers.xavier_initializer(uniform=False)):
	x = tf.layers.conv2d(input_tensor, 64, (7,7), strides=(1,1), padding='SAME', use_bias=False, kernel_initializer=kernel_initializer, name='face_conv1_1/3x3_s1', reuse=reuse)
	...
	x1 = conv_block_2d(x, 3, [64, 64, 256], stage=2, block='face_1a', strides=(2,2), is_training=is_training, reuse=reuse, kernel_initializer=kernel_initializer)
	...
	if pooling_and_fc:
		...
		fc_output      = tf.layers.dense(pooling_output, 512, name='face_fc1', reuse=reuse, kernel_initializer=kernel_initializer)
		fc_output      = tf.layers.batch_normalization(fc_output, training=is_training, reuse=reuse, name='face_fbn')
		fc_output      = tf.nn.l2_normalize(fc_output, axis=-1)
